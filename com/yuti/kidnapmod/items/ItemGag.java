...
public class ItemGag extends ItemKidnapWearable implements IHasModel, ItemUsuableOnRestrainedPlayer, ItemGaggingEffect, IAdjustable {
   public ItemGag(String name, ExtraBondageMaterial materialIn) {
      super(name, materialIn);
      this.func_77637_a(ModCreativeTabs.kidnapTab);
      this.field_77777_bU = 16;
      ModItems.GAG_LIST.add(this);
   }

   public void func_77624_a(ItemStack stack, World world, List<String> addList, ITooltipFlag advanced) {
      addList.add(ChatFormatting.GRAY + I18n.func_135052_a("item.desc.gag", new Object[0]));
      super.func_77624_a(stack, world, addList, advanced);
   }

   public ActionResult<ItemStack> func_77659_a(World p_onItemRightClick_1_, EntityPlayer player, EnumHand hand) {
      ...
      if (player.field_70170_p.field_72995_K) {
         ...
         if (target instanceof EntityPlayer || target instanceof I_Kidnapped) {
            ...
            if (state != null && ((I_Kidnapped)state).isTiedUp()) {
               if (!((I_Kidnapped)state).isGagged()) {
                  ((I_Kidnapped)state).putGagOn(stack);
                  stack.func_190918_g(1);
               } else {
                  ItemStack oldGag = ((I_Kidnapped)state).replaceGag(stack);
                  if (oldGag != null) {
                     stack.func_190918_g(1);
                     ((I_Kidnapped)state).kidnappedDropItem(oldGag);
                  }
               }
            }
         }
   ...
   public ModelBiped getBondageModel(EntityLivingBase entity, ItemStack stack, ExtraBondageItemType slot, ModelBiped modelBase) {
      ...
      float adjustGag = 0.0F;
      if (cap != null) {
         adjustGag = cap.getAdjustement(this.getRegistryName().toString());
      }

      if (entity instanceof EntityDamsel) {
         EntityDamsel damsel = (EntityDamsel)entity;
         adjustGag = damsel.getGagsAdjustement();
      }

      return ModelsUtils.getAdjustement(adjustGag);
   }

   public ExtraBondageItemType getType(ItemStack itemstack) {
      return ExtraBondageItemType.GAG;
   }
