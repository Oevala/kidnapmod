...
public class CommandGag extends CommandTargetPlayer {
   public void func_184881_a(MinecraftServer server, ICommandSender sender, String[] params) throws CommandException {
      ...
      while(var10.hasNext()) {
         ...
         if (state != null && !state.isGagged()) {
            state.putGagOn(new ItemStack(gag, 1, meta));
            Utils.sendValidMessageToEntity(sender, state.getPlayer().func_70005_c_() + " is now gagged.");
            Utils.sendInfoMessageToEntity(state.getPlayer(), "You have been gagged!");
         }
      }
