...
public class CommandUngag extends CommandTargetPlayer {
   public void func_184881_a(MinecraftServer server, ICommandSender sender, String[] params) throws CommandException {
      ...
      while(var5.hasNext()) {
         ...
         if (state != null && state.isGagged()) {
            ItemStack gag = state.takeGagOff();
            EntityPlayer player = state.getPlayer();
            if (gag != null && player != null) {
               player.func_71019_a(gag, true);
            }

            Utils.sendValidMessageToEntity(sender, player.func_70005_c_() + " is now ungagged.");
            Utils.sendValidMessageToEntity(player, "You have been ungagged!");
         }
      }
